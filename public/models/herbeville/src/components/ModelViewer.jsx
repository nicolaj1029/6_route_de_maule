 ...
 * ModelViewer.jsx
 *
 * Mobile-first: 3D never starts automatically.
 * User taps "Activer la vue 3D" → model-viewer loads the GLB.
 * If the GLB path is a placeholder (/models/...) an SVG fallback is shown.
 *
 * Drop Blender GLBs in /public/models/ and update houseOptions.js glb paths.
 */
export default function ModelViewer({ config }) {
  const [active, setActive]   = useState(false)
  const [loading, setLoading] = useState(false)
  const [glbKey, setGlbKey]   = useState(config.house)

  const houseOpt = HOUSE_STYLES.find(h => h.key === glbKey) || HOUSE_STYLES[0]
  const roofOpt  = ROOF_STYLES.find(r => r.key === config.roof) || ROOF_STYLES[0]

  const glbReady = houseOpt.glb && !houseOpt.glb.includes('PLACEHOLDER')
  ...
  const switchModel = (key) => {
    setGlbKey(key)
    const mv = document.getElementById('modelViewerEl')
    const opt = HOUSE_STYLES.find(h => h.key === key)
    if (mv && opt?.glb && !opt.glb.includes('PLACEHOLDER')) {
      mv.setAttribute('src', opt.glb)
    }
  }

  return (
    <div className={styles.wrap}>
      {/* ── Activate button (shown before activation) ── */}
      {!active && !loading && (
        <div className={styles.activate} onClick={activate}>
          ...
          <p className={styles.activateDesc}>
            Modèles GLB/GLTF générés depuis Blender. Rotation, zoom, exploration à 360°.
          </p>
      ...
      {loading && (
        <div className={styles.loading}>
          ...
          <p className={styles.loadingText}>Chargement du modèle…</p>
        </div>
      )}

      {/* ── model-viewer (real GLB) ── */}
      {active && glbReady && (
        <model-viewer
          id="modelViewerEl"
          src={houseOpt.glb}
          alt={`Modèle 3D ${houseOpt.label}`}
          camera-controls
          auto-rotate
          ...
          loading="lazy"
          reveal="auto"
          style={{ width: '100%', height: '320px', background: 'var(--night)' }}
        />
      )}

      {/* ── SVG scene fallback (no GLB yet) ── */}
      {active && !glbReady && (
        <SceneFallback
          houseOpt={houseOpt}
          roofOpt={roofOpt}
      ...
      {/* ── Controls bar (shown after activation) ── */}
      {active && (
        <div className={styles.controls}>
          <div className={styles.glbSelector}>
            {HOUSE_STYLES.map(h => (
              <button
                key={h.key}
                className={`${styles.glbBtn} ${glbKey === h.key ? styles.glbBtnActive : ''}`}
                onClick={() => switchModel(h.key)}
              >
                {h.label}
              </button>
            ))}
          </div>
          <span className={styles.hint}>Glissez pour tourner · Pincez pour zoomer</span>
        </div>
      )}
    </div>
  )
...
function SceneFallback({ houseOpt, roofOpt, hasPiscine, hasLavande }) {
  return (
    <svg
      ...
      aria-label="Visualisation illustrée du terrain"
    >
      <defs>
      ...
      {hasLavande && [350, 362, 374, 386, 398].map((x, i) => (
        <g key={i} opacity="0.9">
          ...
          <ellipse cx={x + 2.5} cy="203" rx="4" ry="6"
            fill={['#8A6AAA','#7A5A9A','#9A7ABA','#8A6AAA','#7A5A9A'][i]} />
        </g>
      ))}
      {/* Label */}
      <text x="250" y="310" textAnchor="middle" fill="rgba(247,244,238,0.2)"
        fontSize="9" fontFamily="monospace" letterSpacing="0.1em">
        MODÈLE SVG · REMPLACER PAR GLB BLENDER
      </text>
    </svg>
  )
}
